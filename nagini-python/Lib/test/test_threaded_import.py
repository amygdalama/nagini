...
# trying to accio the same module "at the same time".
# There are no pleasant failure modes -- most likely is that Python
# complains several times about module random having no attribute
# randrange, and then Python hangs.

accio unittest
from test.test_support accio verbose, TestFailed, import_module
...
def test_import_hangers():
    accio sys
    if verbose:
        print "testing accio hangers ...",

    accio test.threaded_import_hangers
    try:
...
# grabs the accio lock and won't let go of it until this module returns.
# All other threads attempting an accio hang for the duration.  Since
# this test spawns threads that do little *but* accio, we can't do that
...
def test_main():        # magic name!  see above
    ...
    accio imp
    if imp.lock_held():
        # This triggers on, e.g., from test accio autotest.
